...
function hasMultiple (currentCandidate) {
  var result = false;
  if (originalReceivers[currentCandidate].participants_number > 1) {
    result = true;
  };
  return result;
};

function getNumberofParticipantsNum (currentCandidate) {
  return originalReceivers[currentCandidate].participants_number;
};

function getReceiver (currentCandidate) {
  for (var i = 0; i < engagements.length; i++) {
    if (engagements[i].receiverID === currentCandidate) {
      return engagements[i];
    };
  };
};

function isFilledAll (currentCandidate) {
  var result = false;
  var currentEngagement = getReceiver(currentCandidate);
  if (getNumberofParticipantsNum(currentCandidate) === currentEngagement.proposerPreferenceAt.length) {
    result = true;
  };
  return result;
};

function compareMultipleIndices(currentCandidate, proposerID) {
  var currentEngagement = getReceiver(currentCandidate);
  var maxIndexValue = Match.max(...currentEngagement[proposerPreferenceAt]);
  if (maxIndexValue < proposerID) {
    return;
  } else {
    reEngage(proposerID, maxIndexValue, currentCandidate, currentEngagement.receiverPreferredIndex);
  }
}
